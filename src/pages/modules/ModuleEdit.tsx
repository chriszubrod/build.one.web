...
export default function ModuleEdit() {
  ...
  if (item && !form) {
    setForm({ name: item.name, route: item.route ?? "", row_version: item.row_version });
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaveError("");
    try {
      await updateEntity(`/api/v1/update/module/${id}`, {
  ...
  return (
    <div className="page">
      <div className="page-header"><h1>Edit Module</h1></div>
      <form className="form-card" onSubmit={handleSubmit}>
        ...
        <div className="form-actions">
          ...
          <button type="button" className="btn btn-secondary" onClick={() => navigate(`/module/${id}`)}>Cancel</button>
        </div>
      </form>
    </div>
  );
