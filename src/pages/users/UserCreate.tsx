...
export default function UserCreate() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaveError("");
    try {
      const created = await createEntity<User>("/api/v1/create/user", {
        firstname: form.firstname,
        lastname: form.lastname || null,
      });
      navigate(`/user/${created.public_id}`);
    } catch (err: any) {
    ...
    <div className="page">
      ...
      <form className="form-card" onSubmit={handleSubmit}>
        ...
        <div className="form-actions">
          ...
          <button type="button" className="btn btn-secondary" onClick={() => navigate("/user/list")}>Cancel</button>
        </div>
      </form>
    </div>
  );
