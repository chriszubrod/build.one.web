...
export default function UserProjectEdit() {
  ...
  if (item && !form) {
    setForm({ user_id: item.user_id, project_id: item.project_id, row_version: item.row_version });
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaveError("");
    try {
      await updateEntity(`/api/v1/update/user_project/${id}`, {
  ...
  return (
    <div className="page">
      <div className="page-header"><h1>Edit User Project</h1></div>
      <form className="form-card" onSubmit={handleSubmit}>
        ...
        <div className="form-actions">
          ...
          <button type="button" className="btn btn-secondary" onClick={() => navigate(`/user-project/${id}`)}>Cancel</button>
        </div>
      </form>
    </div>
  );
