...
export default function LaborList() {
  ...
  const pendingQuery = useQuery<ContractLabor[]>({
    queryKey: ["contract-labor", "pending_review"],
    queryFn: async () =>
      (await getList<ContractLabor>(`/api/v1/contract-labor/by-status/pending_review`)).data,
  });

  const sorted = useMemo<ContractLabor[]>(() => {
    return (pendingQuery.data ?? []).slice().sort((a, b) => {
  ...
  return (
    <div className="ios-page">
      ...
      <div className="section-label-prose">{STATUSES[0].label}</div>
