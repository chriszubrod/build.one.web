...
import SubCostCodeCreate from "./SubCostCodeCreate";
...
const LIST_PATH = "/api/v1/get/sub-cost-codes";
const ITEM_PATH = `/api/v1/get/sub-cost-code/${PUBLIC_ID}`;
...
vi.mock("../../hooks/useEntity", () => ({
  ...
  useEntityList: (path: string) => mockUseEntityList(path),
...
beforeEach(() => {
  ...
  mockCreateEntity.mockResolvedValue(sampleSubCostCode());
...
describe("sub-cost-code cache invalidation", () => {
  it("create invalidates list only", async () => {
    ...
    const costCodeSelect = container.querySelector('select[name="cost_code_public_id"]') as HTMLSelectElement;
    setInputValue(numberInput, "100.1");
    setInputValue(nameInput, "General Conditions Detail");
    await act(async () => {
      costCodeSelect.value = PARENT_COST_CODE_PUBLIC_ID;
      costCodeSelect.dispatchEvent(new Event("change", { bubbles: true }));
    });
  ...
  it("edit invalidates list and item", async () => {
    ...
    await flushUntil(() => container.querySelector("form") !== null);
  ...
  it("delete removes list and item from cache", async () => {
    ...
    renderPage(
      createElement(SubCostCodeView),
      `/sub-cost-code/${PUBLIC_ID}`,
