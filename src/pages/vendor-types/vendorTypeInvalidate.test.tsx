...
const LIST_PATH = "/api/v1/get/vendor-types";
const ITEM_PATH = `/api/v1/get/vendor-type/${PUBLIC_ID}`;
...
beforeEach(() => {
  ...
  mockCreateEntity.mockResolvedValue(sampleVendorType());
...
describe("vendor-type cache invalidation", () => {
  it("create invalidates list only", async () => {
    renderPage(createElement(VendorTypeCreate), "/vendor-type/create", "/vendor-type/create");
  ...
  it("edit invalidates list and item", async () => {
    renderPage(
      createElement(VendorTypeEdit),
      `/vendor-type/${PUBLIC_ID}/edit`,
  ...
  it("delete removes list and item from cache", async () => {
    ...
    renderPage(
      createElement(VendorTypeView),
      `/vendor-type/${PUBLIC_ID}`,
