...
const COMPANY_GET_PATH = "/api/v1/get/company/abc123";
...
// /api/v1/get/company/undefined).
describe("company route param -> fetch path", () => {
  const cases: [string, ComponentType, string, string][] = [
    ["CompanyView", CompanyView, "/company/abc123", "/company/:publicId"],
    ["CompanyEdit", CompanyEdit, "/company/abc123/edit", "/company/:publicId/edit"],
  ];
  ...
  it.each(cases)("%s fetches using publicId from the route", (_name, Component, initialEntry, routePath) => {
    ...
    expect(mockUseEntityItem).toHaveBeenCalledWith(COMPANY_GET_PATH);
  });
});
