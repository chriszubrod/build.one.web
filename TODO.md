...
- [ ] **Tier 2 decision gate — offline reads.** PWA Tier 1 shipped 2026-06-15 (commits `e37793d`, `19b9d7e`, and follow-ups). The evaluation memo (workflow run in 2026-06-15 session) recommended committing to Tier 2 the same quarter — ~4 weeks for persistent React Query cache via `@tanstack/react-query-persist-client` + `idb-keyval`, user-scoped persister key + awaited cleanup on logout (the iOS v0.1.0 multi-user state-bleed lesson lifted to web), and an OfflineBanner that surfaces "last synced X ago." Concrete trigger: once Tier 1 is stable in prod for ~1 week with no SW regressions, kick off Tier 2 spec.
