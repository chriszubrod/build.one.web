...
- [ ] **Budgets is unreachable from nav — same bug class U-046 just fixed.** `Modules.BUDGETS` exists and `/budget/list|create|:publicId|:publicId/edit` are all routed, but there is **no `MENU_ENTRIES` entry for budgets**. It is linked only from `BudgetSidebar`, which renders only once you are already on `/budget/*` — so Budgets is reachable **only by typing the URL**. Verified 2026-07-16. Adding the entry is a behavior change + a per-role curation decision, so it was left out of U-046. Pairs with the BudgetLayout retirement item above.
- [ ] **Nav has no shell-awareness — `MenuEntry` declares `route` but not which layout owns it.** U-046 newly exposes More → Financials → **Bills** on a phone, but `/bill/*` renders under `BillLayout`, whose `.sidebar` is a hard `width: 220px; flex-shrink: 0` with **no media query** — so a 220px sidebar eats a 430px column, with no bottom pill and no More drawer. Not a dead-end (the `← Field App` link escapes) and not a regression (AppSidebar already exposed Bills at ≥768px), but it is a real phone→desktop-chrome jump. Right-depth fixes: a `shell`/`surface` field on `MenuEntry`, or fold Bill/Budget into the responsive `AppLayout` (see the BudgetLayout retirement item). Do not paper over it per-entry.
...
- [ ] **`BillSidebar` / `BudgetSidebar` are hand-maintained nav surfaces outside `menuConfig`.** Each hardcodes its own `<NavLink>` list — the same shape as the pre-U-046 `AppSidebar` that caused this bug. Fold into the menuConfig model when retiring those layouts.
